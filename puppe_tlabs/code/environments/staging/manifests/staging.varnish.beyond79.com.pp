node "staging.varnish.drupal.beyond79.com" {
   ...
   # end repos
    ...
    varnish::vcl { '/etc/varnish/default.vcl':
        content => template('varnish/staging-drupal.vcl.erb'),
    }
    #end varnish

    # nginx (to proxy to internal-staging-http-drupal-sites-1267534014.us-west-2.elb.amazonaws.com)
    class {
        'nginx':
            sendfile => 'off',
            worker_processes => '2',
    ...
    nginx::resource::vhost { 'default_server':
        listen_port => 8080,
        proxy       => 'http://internal-staging-http-drupal-sites-1267534014.us-west-2.elb.amazonaws.com:80',
    }
    # end nginx
}